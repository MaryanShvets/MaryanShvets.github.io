<?
	...
	$traffic_all_history = mysql_query(" 
		...
		FROM `app_events` 
		WHERE `url` = 'http://pages.polza.com/kir/money/code' 
		AND `date` >= '$old' 
	...
	$lead_all_history = mysql_query(" 
		...
		FROM `app_leads` 
		WHERE `product` IN('350', '351', '352', '353', '354')   
		AND `data`>= '$old' 
	...
	$visitors_lp = API::query( "SELECT COUNT(DISTINCT(`user`)) as `count`  FROM `app_events` WHERE `url` = 'http://pages.polza.com/kir/money/code' AND `date`>= '$old'  " );
	$visitors_price = API::query( "SELECT COUNT(DISTINCT(`user`)) as `count`  FROM `app_events` WHERE `url` = 'http://pages.polza.com/kir/money/code-price' AND `date`>= '$old'  " );
	$visitors_form = API::query( "SELECT COUNT(DISTINCT(`user`)) as `count`  FROM `app_events` WHERE `url` IN('http://polza.com/pages/form/350','http://polza.com/pages/form/351','http://polza.com/pages/form/352')  AND `date`>= '$old' " );
	$visitors_lead = API::query( "SELECT COUNT(DISTINCT(`contact`)) as `count`  FROM `app_leads` WHERE `product` IN('342', '343', '344')   AND `data`>= '$old'  " );
	$visitors_paid = API::query( " SELECT COUNT(*)  as `count` FROM `app_payments` WHERE `order_id` IN(SELECT `id` FROM `app_leads` WHERE `product` IN('350', '351', '352', '353', '354')) AND `status` = 'success'   AND `date_create`>= '$old'  " );

	$visitors_lp_funnel = mysql_query( "SELECT `utm_source`, COUNT(DISTINCT(`user`)) as `count` FROM `app_events` WHERE `url` = 'http://pages.polza.com/kir/money/code' AND `date`>= '$old' GROUP BY `utm_source`" );
	...
	$visitors_form_funnel = mysql_query( "
			SELECT `utm_source`, COUNT(DISTINCT(`user`)) as `count` FROM `app_events` WHERE `user` IN(
					SELECT `user`  
						...
						WHERE 
							`url` IN (
									'http://polza.com/pages/form/350',
									'http://polza.com/pages/form/351',
									'http://polza.com/pages/form/352'
								)
				)
			AND `url` = 'http://pages.polza.com/kir/money/code' AND `date`>= '$old' GROUP BY `utm_source`" );

	$visitors_price_funnel = mysql_query( "
			SELECT `utm_source`, COUNT(DISTINCT(`user`)) as `count` FROM `app_events` WHERE `user` IN(
					SELECT `user`
						FROM `app_events` 
						WHERE `url` = 'http://pages.polza.com/kir/money/code-price' AND `date`>= '$old'
				)
			AND `url` = 'http://pages.polza.com/kir/money/code' AND `date`>= '$old' GROUP BY `utm_source`" );

	$visitors_lead_funnel = mysql_query(" SELECT `utmsourse` as `utm_source`, COUNT(*) as `count` FROM `app_leads` WHERE `product` IN('350', '351', '352') AND `data`>= '$old' GROUP BY `utm_source` ");

	$visitors_paid_funnel = mysql_query("
		SELECT `utmsourse` as `utm_source`, COUNT(*) as `count` FROM `app_leads` WHERE `id` IN(
			SELECT `order_id` FROM `app_payments` WHERE `order_id` IN(SELECT `id` FROM `app_leads` WHERE `product` IN('350', '351', '352')) AND `status` = 'success'   AND `date_create`>= '$old'
		) GROUP BY `utm_source`
	");
...
<div class="container dashboard" style="max-width: 1800px;margin-left: 20px;margin-right: 20px;">
