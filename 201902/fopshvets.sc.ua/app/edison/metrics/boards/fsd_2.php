...
<?
	...
	$traffic_all_history = mysql_query(" 
		...
		FROM `app_events` 
		WHERE `url` = 'http://pages.polza.com/kir/money/fsd' 
		AND `date` >= '$old' 
	...
	$visitors_lp = API::query( "SELECT COUNT(DISTINCT(`user`)) as `count`  FROM `app_events` WHERE `url` = 'http://pages.polza.com/kir/money/fsd' AND `date`>= '$old'  " );
	$visitors_price = API::query( "SELECT COUNT(DISTINCT(`user`)) as `count`  FROM `app_events` WHERE `url` = 'http://pages.polza.com/kir/money/fsd-price' AND `date`>= '$old'  " );
	$visitors_form = API::query( "SELECT COUNT(DISTINCT(`user`)) as `count`  FROM `app_events` WHERE `url` IN('http://polza.com/pages/form/341','http://polza.com/pages/form/340','http://polza.com/pages/form/339','http://polza.com/pages/form/338','http://polza.com/pages/form/337','http://polza.com/pages/form/336')  AND `date`>= '$old' " );
	$visitors_lead = API::query( "SELECT COUNT(DISTINCT(`contact`)) as `count`  FROM `app_leads` WHERE `product` IN('336', '337', '338','339','340', '341')   AND `data`>= '$old'  " );
	$visitors_paid = API::query( " SELECT COUNT(*)  as `count` FROM `app_payments` WHERE `order_id` IN(SELECT `id` FROM `app_leads` WHERE `product` IN('336', '337', '338','339','340', '341')) AND `status` = 'success'   AND `date_create`>= '$old'  " );

	$visitors_lp_funnel = mysql_query( "SELECT `utm_source`, COUNT(*) as `count` FROM `app_events` WHERE `url` = 'http://pages.polza.com/kir/money/fsd' AND `date`>= '$old' GROUP BY `utm_source`" );
	...
	$visitors_form_funnel = mysql_query( "
			SELECT `utm_source`, COUNT(DISTINCT(`user`)) as `count` FROM `app_events` WHERE `user` IN(
					SELECT `user`  
						...
						WHERE 
							`url` IN (
									'http://polza.com/pages/form/336',
									'http://polza.com/pages/form/337',
									'http://polza.com/pages/form/338'
								)
				)
			AND `url` = 'http://pages.polza.com/kir/money/fsd' AND `date`>= '$old' GROUP BY `utm_source`" );

	$visitors_price_funnel = mysql_query( "
			SELECT `utm_source`, COUNT(DISTINCT(`user`)) as `count` FROM `app_events` WHERE `user` IN(
					SELECT `user`
						FROM `app_events` 
						WHERE `url` = 'http://pages.polza.com/kir/money/fsd-price' AND `date`>= '$old'
				)
			AND `url` = 'http://pages.polza.com/kir/money/fsd' AND `date`>= '$old' GROUP BY `utm_source`" );

	$visitors_lead_funnel = mysql_query(" SELECT `utmsourse` as `utm_source`, COUNT(*) as `count` FROM `app_leads` WHERE `product` IN('336', '337', '338','339','340', '341') AND `data`>= '$old' GROUP BY `utm_source` ");

	$visitors_paid_funnel = mysql_query("
		SELECT `utmsourse` as `utm_source`, COUNT(*) as `count` FROM `app_leads` WHERE `id` IN(
			SELECT `order_id` FROM `app_payments` WHERE `order_id` IN(SELECT `id` FROM `app_leads` WHERE `product` IN('336', '337', '338','339','340', '341')) AND `status` = 'success'   AND `date_create`>= '$old'
		) GROUP BY `utm_source`
	");
...
<div class="grid-padded">
	...
	<div class="grid">
		...
		<div class="col block col-9 ">
			<h3 class="ui-chart-subheader">Участников / Выручка</h3>
			<div class="chart" id="traffic_all_history" style="height: 150px;"></div>
			<div class="chart" id="people_all_history" style="height: 150px;"></div>
			<div class="chart" id="money_all_history" style="height: 150px;"></div>
		</div>
	...
	<div class="grid">
		<div class="col block">
			...
			<div class="chart" id="funnel" style="min-height: 700px;"></div>
			<!-- <div id="stacked" ></div> -->
