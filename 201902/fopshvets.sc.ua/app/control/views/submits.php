		...
		<h1>Регистрации</h1>
			...
			<?
				...
				if(!empty($_GET['product'])){
					$sql_product = "WHERE `product` = '".$_GET['product']."'";
				}else{
					$sql_product = "";
				}

				$count = $api->query(" SELECT COUNT(*) as count FROM `app_leads` $sql_product ");
				$per_page = 10;
				...
				$data = mysql_query(" SELECT * FROM `app_leads` $sql_product ORDER BY `app_leads`.`id`  DESC LIMIT $start, $per_page ") or die(mysql_error());
				...
				if($pages>2){
					...
					if ($cur_page == 1) {
						$i = $cur_page + 1;
						echo '<a href="?view=submits'.$pag_pro.'&page='.$i.'">Следующая</a> ';
						echo '<a href="?view=submits'.$pag_pro.'&page='.$pages.'">Конец</a> ';
					}elseif($cur_page !== 1 && $cur_page < $pages){
						$p = $cur_page - 1;
						$n = $cur_page + 1;
						echo '<a href="?view=submits'.$pag_pro.'&page=1">Начало</a> ';
						echo '<a href="?view=submits'.$pag_pro.'&page='.$p.'">Предыдущая</a> ';
						echo '<a href="?view=submits'.$pag_pro.'&page='.$n.'">Следующая</a> ';
						echo '<a href="?view=submits'.$pag_pro.'&page='.$pages.'">Конец</a> ';
					}else{
						$p = $cur_page - 1;
						echo '<a href="?view=submits'.$pag_pro.'&page=1">Начало</a> ';
